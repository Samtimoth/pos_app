...
- ❌ **Bado hakuna**: credit notes rasmi (returns zinaonyesha refund lakini hazitoi hati ya "credit note" tofauti), split payment kwa `slow_payment` haijajaribiwa live (logic ipo lakini haijapata real-world test), Checkout widget moja (desktop `_CartPanel` + simu `_CartSheet` bado mbili — uamuzi wa makusudi kuahirisha).
...
1. **Hatua 2 — kilichobaki**: (a) ~~customer_id kwenye tbl_sales~~ ✅; (b) ~~backfill ya sales za zamani~~ ✅ (imethibitishwa hakuna kazi zaidi, 2026-09-16); (c) ~~sale_payments (split + change)~~ ✅ (credit note rasmi bado); (d) ~~returns/refunds~~ ✅; (e) Checkout widget moja (bado imeahirishwa kwa makusudi) — **Hatua 2 kwa vitendo iko karibu kukamilika kabisa.**
