...
## Hatua 3 — Stock ledger: 🟡 IMEENDELEA ZAIDI
...
- ❌ Bado: StockService moja kwa writes ZOTE (sales/void/transfer bado zinabadilisha `tbl_product.stock` moja kwa moja, si kupitia huduma moja ya kati — ingawa trigger `trg_product_au` inarekodi ledger kwa ZOTE tayari, hivyo audit trail ipo, ni usanifu tu ambao haujaunganishwa), opening-balance backfill (tayari ipo kwa bidhaa mpya kupitia `trg_product_ai`, lakini bidhaa za zamani kabla ya ledger — hali haijulikani, angalia `stock_movements` movement_type='opening' counts), approval ya adjustments (mtu yeyote mwenye `stock.adjust` anaweza kurekebisha bila idhini ya pili).
...
2. **Hatua 3 — kilichobaki**: ~~batches mbili → moja~~ ✅, ~~FEFO~~ ✅ (zote 2026-09-16). Kimebaki: StockService moja (usanifu tu, si bug — ledger ipo tayari kwa writes zote kupitia trigger), opening backfill kwa bidhaa za zamani (haijathibitishwa), approval ya adjustments.
