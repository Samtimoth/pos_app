...
UPDATE products SET category='kienyeji' WHERE LOWER(name) LIKE '%kienyeji%';
UPDATE products SET category='broiler' WHERE LOWER(name) LIKE '%broiler%';
UPDATE products SET category='mayai' WHERE LOWER(name) LIKE '%mayai%';
UPDATE products SET category='vifaranga' WHERE LOWER(name) LIKE '%vifaranga%';
...
SELECT
  'Mtwesa Poultry','Kuku wa Kienyeji Wamefika!',
...
SELECT
  'Mtwesa Broiler Farm','Broiler Tayari kwa Oda',
...
WHERE NOT EXISTS (
  SELECT 1 FROM advertisements WHERE title='Broiler Tayari kwa Oda'
);
